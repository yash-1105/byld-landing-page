    ...
    <section id="product" style={{ scrollMarginTop: 90, background: '#F1ECE2', borderTop: '1px solid #E8E2D6', borderBottom: '1px solid #E8E2D6' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: 'clamp(80px,10vw,140px) 40px' }}>
        ...
        <div data-reveal="1" data-reveal-delay="0.08" style={{ background: '#FCFAF6', border: '1px solid #E6E0D4', borderRadius: 22, overflow: 'hidden', boxShadow: '0 2px 6px rgba(41,38,31,.03), 0 50px 90px -50px rgba(41,38,31,.35)' }}>
          {/* app chrome */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 18px', borderBottom: '1px solid #EDE7DB', background: '#FAF7F1' }}>
            <div style={{ display: 'flex', gap: 7 }}>
              {[0, 1, 2].map((i) => <span key={i} style={{ width: 11, height: 11, borderRadius: '50%', background: '#E0D7C7' }} />)}
            </div>
            <div style={{ flex: 1, maxWidth: 460, margin: '0 auto', background: '#F1ECE2', border: '1px solid #E6E0D4', borderRadius: 8, padding: '6px 14px', fontFamily: mono, fontSize: 11, color: '#8B8275', textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{viewTitle}</div>
            <div style={{ display: 'flex' }}>
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#C7A574', border: '2px solid #FAF7F1' }} />
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#9CA382', border: '2px solid #FAF7F1', marginLeft: -7 }} />
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#B0A18C', border: '2px solid #FAF7F1', marginLeft: -7 }} />
            </div>
          </div>

          <div style={{ display: 'flex', minHeight: 486, flexWrap: 'wrap' }}>
            {/* sidebar */}
            <div style={{ flex: 'none', width: 214, borderRight: '1px solid #EDE7DB', padding: '18px 14px', background: '#FAF7F1', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '0 8px 18px' }}><img src="/assets/byld-logo.png" alt="BYLD" style={{ height: 17, display: 'block' }} /></div>
              <div style={{ fontFamily: mono, fontSize: 9, letterSpacing: '.14em', color: '#A79E90', padding: '0 11px 9px' }}>WORKSPACE</div>
              ...
              <div style={{ marginTop: 'auto', padding: '14px 11px 0', borderTop: '1px solid #EDE7DB' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, background: 'linear-gradient(135deg,#C7A574,#B08E5C)' }} />
                  <div><div style={{ fontSize: 12, fontWeight: 600, color: '#29261F' }}>Studio Atelier</div><div style={{ fontFamily: mono, fontSize: 9, color: '#A79E90' }}>8 PROJECTS</div></div>
                </div>
              </div>
            </div>
...
function PMPanel() {
  ...
  const proj = (scene: SceneKind, name: string, w: string, color: string, pct: string) => (
    <div className="hover-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 8px', borderRadius: 9 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600, color: '#29261F' }}>{name}</div><div style={{ height: 5, width: '100%', background: '#EAE3D5', borderRadius: 3, marginTop: 7 }}><div style={{ width: w, height: '100%', background: color, borderRadius: 3 }} /></div></div>
      <span style={{ fontSize: 10.5, color: '#8B8275', fontFamily: mono }}>{pct}</span>
    </div>
  )
  const act = (dot: string, text: string, time: string) => (
    <div style={{ display: 'flex', gap: 10, marginBottom: 13 }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: dot, marginTop: 4, flex: 'none' }} /><div><div style={{ fontSize: 12, color: '#29261F' }}>{text}</div><div style={{ fontFamily: mono, fontSize: 9, color: '#A79E90', marginTop: 2 }}>{time}</div></div></div>
  )
  return (
    <div style={panelIn}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>{headTag('MONDAY · 17 JUN')}{h3('Studio overview')}</div>
        <div className="newproj btn-dark" data-mag="0.3" style={{ background: '#29261F', color: '#F6F4EF', fontSize: 12.5, fontWeight: 500, padding: '9px 16px', borderRadius: 9, cursor: 'pointer' }}>+ New project</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 18 }}>
        {stat('ACTIVE PROJECTS', '8', '#29261F')}
        {stat('PENDING APPROVALS', '3', '#B17A57')}
        {stat('ON SITE TODAY', '2', '#29261F')}
      </div>
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
        <div style={{ flex: '1.6', minWidth: 280, background: '#FCFAF6', border: '1px solid #EDE7DB', borderRadius: 13, overflow: 'hidden' }}>
          <div style={{ padding: '12px 15px', borderBottom: '1px solid #F0EADF', fontSize: 12.5, fontWeight: 600, color: '#29261F' }}>Active projects</div>
          <div style={{ padding: '6px 8px' }}>
            {proj('facade', 'Riverside Residence', '72%', '#29261F', '72%')}
            {proj('interior', 'Marigold Café Interior', '40%', '#B17A57', '40%')}
            {proj('house', 'Hillcrest Villa', '18%', '#C9BFAE', '18%')}
          </div>
        </div>
        <div style={{ flex: 1, minWidth: 220, background: '#FCFAF6', border: '1px solid #EDE7DB', borderRadius: 13, padding: 14 }}>
  ...
  return (
    <div style={panelIn}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(252px,1fr))', gap: 12 }}>
        {PROC.map((it) => (
          <div key={it.id} style={{ background: '#FCFAF6', border: '1px solid #EDE7DB', borderRadius: 14, padding: 14 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 11 }}>
              <div style={{ fontSize: 13.5, fontWeight: 600, color: '#29261F' }}>{it.name}</div>
              <div style={{ fontFamily: mono, fontSize: 9, color: '#A79E90' }}>{it.sub}</div>
            </div>
              ...
              {it.vendors.map((v, vi) => {
                ...
                return (
                  <div key={v.n} role="button" tabIndex={0}
                    ...
                    aria-pressed={sel}
                    style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: 10, padding: '11px 13px', borderRadius: 11, cursor: 'pointer', border: '1px solid ' + (sel ? '#29261F' : '#EDE7DB'), background: sel ? '#FCFAF6' : '#F7F3EB', transition: 'border-color .25s, background .25s, transform .25s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
                      <span style={{ width: 16, height: 16, borderRadius: '50%', flex: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, lineHeight: 1, background: sel ? '#29261F' : 'transparent', color: '#F6F4EF', border: sel ? 'none' : '1px solid #CFC4B0' }}>{sel ? '✓' : ''}</span>
                      <div style={{ minWidth: 0 }}><div style={{ fontSize: 12.5, fontWeight: 500, color: '#29261F', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{v.n}</div><div style={{ fontFamily: mono, fontSize: 9, color: '#A79E90', marginTop: 2 }}>★ {v.rating} · {v.lead}</div></div>
                    </div>
...
function DocsPanel() {
  const scatter = (style: React.CSSProperties, name: string) => (
    <div style={{ position: 'absolute', background: '#FCFAF6', border: '1px solid #EDE7DB', borderRadius: 8, padding: '8px 11px', fontSize: 10.5, color: '#8B8275', boxShadow: '0 8px 18px -10px rgba(41,38,31,.25)', ...style }}>{name}</div>
  )
  const settle = (from: string, delay: string, tone: string, name: string, tag: string, tagColor: string) => (
    <div style={{ '--from': from, animation: `settle .8s cubic-bezier(.2,.7,.3,1) both`, animationDelay: delay, display: 'flex', alignItems: 'center', gap: 11, background: '#F4EFE6', borderRadius: 9, padding: '9px 11px' } as React.CSSProperties}>
  ...
  return (
    <div style={panelIn}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 200, position: 'relative', height: 232, border: '1px dashed #D8CDBA', borderRadius: 14, background: '#F7F3EB' }}>
          <div style={{ position: 'absolute', left: 14, top: 12, fontFamily: mono, fontSize: 9, letterSpacing: '.1em', color: '#A79E90' }}>SCATTERED</div>
        ...
        <div style={{ flex: 'none', color: '#B17A57', fontFamily: mono, fontSize: 20 }}>→</div>
...
function ApprovalsPanel({ approvals, historyOpen, setStatus, toggleHistory }: {
  ...
  const chipFor = (s: ApprovalStatus) =>
    s === 'approved' ? { t: 'Approved', bg: '#EBF0E2', c: '#5E6A4D' }
      : s === 'rejected' ? { t: 'Changes requested', bg: '#F3E2DD', c: '#A4593F' }
        : { t: 'Pending', bg: '#F3EAD9', c: '#A77B3F' }
  return (
    <div style={panelIn}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
        {approvals.map((a) => {
          ...
          const border = a.status === 'approved' ? '#D7E0C6' : a.status === 'rejected' ? '#E8D2CA' : '#EDE7DB'
          return (
            <div key={a.id} style={{ background: '#FCFAF6', border: '1px solid ' + border, borderRadius: 14, padding: 15, transition: 'border-color .35s, opacity .35s, transform .35s' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 11, paddingTop: 11, borderTop: '1px solid #F0EADF' }}>
                <button className="btn-history" onClick={() => toggleHistory(a.id)} aria-expanded={historyOpen === a.id} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: mono, fontSize: 10, letterSpacing: '.08em', color: '#8B8275', padding: 0 }}>{historyOpen === a.id ? '▴' : '▾'} VIEW HISTORY</button>
              </div>
              {historyOpen === a.id && (
                <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8, animation: 'panelIn .35s ease both' }}>
                  {a.history.map((h, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9 }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: '#C9BFAE', flex: 'none' }} /><span style={{ fontSize: 11.5, color: '#5C564B' }}>{h}</span></div>
                  ))}
                </div>
              )}
            </div>
...
function SitePanel() {
  ...
  return (
    <div style={panelIn}>
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
        <div style={{ flex: '1.3', minWidth: 260, position: 'relative', height: 262, borderRadius: 14, overflow: 'hidden', border: '1px solid #EDE7DB' }}>
          <div style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(135deg,#EAE3D5 0 13px,#E4DCCB 13px 26px)' }} />
          <div style={{ position: 'absolute', left: 16, top: 14, fontFamily: mono, fontSize: 9.5, letterSpacing: '.1em', color: '#8B8275' }}>RIVERSIDE · PHASE 02</div>
          <div className="site-pill" style={{ position: 'absolute', left: 32, top: 80, display: 'flex', alignItems: 'center', gap: 8, background: '#FCFAF6', padding: '8px 13px', borderRadius: 999, boxShadow: '0 8px 20px -10px rgba(41,38,31,.35)', cursor: 'default' }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#7E866A' }} /><span style={{ fontSize: 11.5, color: '#29261F', fontWeight: 500 }}>Foundation poured</span></div>
          <div className="site-pill" style={{ position: 'absolute', right: 28, top: 130, display: 'flex', alignItems: 'center', gap: 8, background: '#FCFAF6', padding: '8px 13px', borderRadius: 999, boxShadow: '0 8px 20px -10px rgba(41,38,31,.35)', cursor: 'default' }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#B17A57' }} /><span style={{ fontSize: 11.5, color: '#29261F', fontWeight: 500 }}>Steelwork in progress</span></div>
          <div style={{ position: 'absolute', left: 40, bottom: 24, display: 'flex', alignItems: 'center', gap: 8, background: '#29261F', padding: '8px 13px', borderRadius: 999 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#7E866A' }} /><span style={{ fontSize: 11.5, color: '#F6F4EF', fontWeight: 500 }}>3 issues resolved</span></div>
        </div>
